...
def return_args() -> argparse.Namespace:
    ...
    # training params
    parser.add_argument('--dataset', type=str, default='CS', choices=['CiteSeer', 'CS'],
...
def visualise(args):
    ...
    # calculate vectors before classifier layer
    print("Inference representations")
    if args.layer_type == 'GCN':
        h1 = model.layer1(x, edge_index)  # (n_node,nheads*d_hidden)
        h1 = model.LeakyRelu(h1)
    else:
        h1 = model.att_layer1(x, edge_index)  # (n_node,nheads*d_hidden)
        h1 = model.LeakyRelu(h1)

    x = h1[data.test_mask].detach().numpy()
    y = data.y[data.test_mask].detach().numpy()

    import seaborn as sns
    sns.set(context="paper", style="white")

    # Do T-SNE
    print("Conducting UMAP")
    # tsne = manifold.TSNE(n_components=2, init='pca', random_state=0, perplexity=30, n_iter=15000)
    ...
    # plt.show()
    plt.savefig(os.path.join('report', f'{args.layer_type}-{args.dataset}-num_per_class{args.num_train_per_class}.png'))
