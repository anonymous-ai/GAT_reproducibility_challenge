...
# import models.layers_tuan as layers
...
def return_args() -> argparse.Namespace:
    ...
    # training params
    parser.add_argument('--dataset', type=str, default='CiteSeer', choices=['CiteSeer', 'CS'],
...
class PytorchGeoNet(torch.nn.Module):
    def __init__(self,
        ...
        assert layer_type in ['GATTorch', 'GCN']

        if layer_type == 'GATTorch':
            self.layer1 = GATConv(
...
class GATNet(torch.nn.Module):
    ...
    def param_init(self):
        ...
        if self.layer_type == 'GAT':
            torch.nn.init.xavier_uniform_(self.att_layer1.W)
            torch.nn.init.xavier_uniform_(self.att_layer1.a)

            torch.nn.init.xavier_uniform_(self.att_layer2.W)
            torch.nn.init.xavier_uniform_(self.att_layer2.a)
            ...
            torch.nn.init.xavier_uniform_(self.att_layer1.WK)
            torch.nn.init.xavier_uniform_(self.att_layer1.WQ)
            torch.nn.init.xavier_uniform_(self.att_layer1.WV)

            torch.nn.init.xavier_uniform_(self.att_layer2.WK)
            torch.nn.init.xavier_uniform_(self.att_layer2.WQ)
            torch.nn.init.xavier_uniform_(self.att_layer2.WV)
            ...
            # self.att_layer1.W.data.uniform(-INIT,INIT)
            # self.att_layer2.W.data.uniform_(-INIT,INIT)
...
def load_dataset(
    ...
    if args.dataset == 'CiteSeer':
        dataset = datasets.Planetoid(
            ...
            pre_transform=torch_geometric.transforms.AddSelfLoops()
        )
        data = dataset.data
...
def load_model(
    ...
    if 'GAT' in args.layer_type and args.layer_type != 'GATTorch':
        model = GATNet(
